...
reads and carries its lockup, and every public URL in the tracked markdown still
responds. The lockup and the product links live off this repository, so the check
also runs daily and drift surfaces without waiting for a commit. Run it locally
with `bash scripts/validate.sh`. A passing merge to `main` is the release.
